...
void  CI_Init (d_CI *a_CI)
{
   ...
   strcpy (a_CI->cr_EmiFla,"");
   strcpy (a_CI->cr_EmiSmo,"");
   strcpy (a_CI->cr_EmiDuf,"");
   strcpy (a_CI->cr_EmiFull,""); 
   a_CI->f_CriInt = -1;  
   a_CI->f_fCO = a_CI->f_fCO2 = a_CI->f_fCH4 = a_CI->f_fPM25 = a_CI->f_fPM10 = a_CI->f_fNOX = a_CI->f_fSO2 = 0;
   a_CI->f_sCO = a_CI->f_sCO2 = a_CI->f_sCH4 = a_CI->f_sPM25 = a_CI->f_sPM10 = a_CI->f_sNOX = a_CI->f_sSO2 = 0;
   a_CI->f_dCO = a_CI->f_dCO2 = a_CI->f_dCH4 = a_CI->f_dPM25 = a_CI->f_dPM10 = a_CI->f_dNOX = a_CI->f_dSO2 = 0;


/* batch user override equation numbers  */
   strcpy (a_CI->cr_BatchEqu,"No"); 
   a_CI->i_EqLit = e_EqMissing; 
   a_CI->i_EgDufLoa = e_EqMissing; 
   a_CI->i_EqDufDep = e_EqMissing; 
   a_CI->i_EqMSE = e_EqMissing; 
   a_CI->i_EqHerb = e_EqMissing;
   a_CI->i_EqShrub = e_EqMissing; 
...
*  Ret: 1 Yes, 0 NO
*
*  NOTE: 4-2-2012 I I modified the functions below to also check for the 
*        2 letter codes, so that I could use those same 2 letter codes
*        as they come in from species cover .dat file 
* 
{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}*/
int CI_isGrassGroup (d_CI *a_CI)
{
 if ( !xstrcmpi(a_CI->cr_CoverGroup,e_GrassGroup))
   return 1;  
 if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_GrassGroup))
   return 1; 
 return 0;
}
...
{
  if ( !xstrcmpi(a_CI->cr_CoverGroup,e_ShrubGroup))
    return 1; 
  if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_ShrubGroup)) 
    return 1; 

  if ( CI_isShrubGroupChaparral (a_CI) )
    return 1; 

  return 0;
}

int CI_isShrubGroupChaparral (d_CI *a_CI)
{
  if ( !xstrcmpi(a_CI->cr_CoverGroup,e_ShrubGroupChaparral))
    return 1; 
  if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_ShrubGroupChaparral)) 
    return 1; 
  return 0;
}
...
int CI_isPinFlaWoo (d_CI *a_CI){ if ( !xstrcmpi(a_CI->cr_CoverGroup,e_PinFlaWoo)) return 1; if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_PinFlaWoo)) return 1;  return 0;}

int CI_isSageBrush  (d_CI *a_CI){ if ( !xstrcmpi(a_CI->cr_CoverGroup,e_SageBrush))  return 1; if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_SageBrush))  return 1;  return 0;}

int CI_isPocosin    (d_CI *a_CI){ if ( !xstrcmpi(a_CI->cr_CoverGroup,e_Pocosin  ))  return 1; if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_Pocosin  ))  return 1; return 0; }

int CI_isPonderosa  (d_CI *a_CI){ if ( !xstrcmpi(a_CI->cr_CoverGroup,e_Ponderosa))  return 1; if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_Ponderosa))  return 1; return 0;}

int CI_isWhiPinHem  (d_CI *a_CI){ if ( !xstrcmpi(a_CI->cr_CoverGroup,e_WhiPinHem))  return 1; if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_WhiPinHem))  return 1; return 0;}

int CI_isRedJacPin  (d_CI *a_CI){ if ( !xstrcmpi(a_CI->cr_CoverGroup,e_RedJacPin))  return 1; if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_RedJacPin))  return 1; return 0;}

int CI_isBalBRWSpr  (d_CI *a_CI){ if ( !xstrcmpi(a_CI->cr_CoverGroup,e_BalBRWSpr))  return 1; if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_BalBRWSpr))  return 1; return 0;}

/* test-ld larry */
int CI_isCoastPlain (d_CI *a_CI){ if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CoastPlain)) return 1; if ( !xstrcmpi(a_CI->cr_CoverGroup,e_CVT_CoastPlain)) return 1; return 0;}
