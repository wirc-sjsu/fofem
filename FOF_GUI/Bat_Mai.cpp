...
int _SetEmisGroup (d_CI *a_CI, char cr_EmiFla[], char cr_EmiSmo[], char cr_EmiDuf[], char cr_EmiFull[], char cr_ErrMes[]);
int Chk_SIF_PFI (d_SIF *a_SIF, d_PFI *a_PFI, char cr[]);
int BIF_Switch_UsrEqu ();
int _ChkPMCut (float f_pPMCut, char cr_ErrMes[]) ;
int PFI_Calc_Old2 (d_PFI *PFI, char cr_ErrMes[])
{
 myMessageBox ("FIX, PFI_Calc_Old, Bat_Mai.cpp"); 
 return 1; 
}
...
*
* Note-6: this value gets used in function  _FireInt_DuffType () bur_brn.cpp
*         to help calc fire intensity, we use "" here so the function will
*         just use the default. This cr_DuffType normally comes from the Settings
*          Window dropdown box, So here in the batch we need to set it before
*          doing burnup and then the soil if we need to do that too
*
* Note-7:   4/9/2018 - change requested by DL
*         When the batch is running Consumed Soil with User Equations the duff
*          percent consumed will come from the Duff Depth Reductions equation
*          and feed into the Soil model. 
*         When User Equations are not being used the duff percent consumed
*          will come from the Duff Consumed equation. 
*         Note-> the GUI always runs using the Duff Consumed Percent, the
*          duff depth reduction equations aren't even used. The only time the
*          duff depth reduction equations ever get used is in the batch when
*          the User Equation option is set and and equations number is present
*          
...
{
int i, iN_Errs, iN_Cnt, i_Equ;
float f; 
char cr_StdId[eC_StdId], cr_1kFmt[30], cr[1000];
char  cr_BrnParFN[300], cr_EmisSw[30];
char cr_ErrMes[1000], cr_FN[1000], cr_EmiFull[30];
char cr_EmiFla[100], cr_EmiSmo[100], cr_EmiDuf[100], cr_BatchEqu[100];
d_CI s_CI;
...
d_SO s_SO;
  ...
  *ai_SoilErr  = 0;
  ...
  strcpy (cr_DuffType,"");    /* See Note-6 above */
...
/*..........................................................*/
/* User Equations Number Switch */ 
char cr_UsrEqu[100];
   if ( BIF_Switch_UsrEqu () )
      strcpy (cr_BatchEqu,"Yes");
   else
      strcpy (cr_BatchEqu, "No"); 

/*........................................................*/
/* Get the Emission Group Numbers from input batch file */
/*  they will get used below */
   if ( !BIF_SwitchEG (cr_EmiFla, cr_EmiSmo, cr_EmiDuf, cr_EmiFull, gcr_tmp)) {
    if ( fh_Err != NULL )
      fprintf (fh_Err,"%s\n",gcr_tmp);
    else
      ErrBat_Window (gcr_tmp, "BAT_CESExec()");
...
/* if there was a Burnup Parameter File Name switch in the Batch input file */
  ...
  if ( iS_CL == 1 )
     BOF_PutHeading (iS_Soil, cr_EmiFla, cr_EmiFull, cr_BatchEqu);          /* Put heading labels to output file */
...
/*   gets initialized for each new plot, see in BIG_Get(...) above */ 
  if ( !stricmp (cr_EmiFla,"") ) {    /* Expanded Factor switchs Not found */  
     s_CI.f_CriInt = -1;              /* so tell Burnup to use Old Defaults */
     goto ChkCon; }

   if ( ! _SetEmisGroup (&s_CI, cr_EmiFla, cr_EmiSmo, cr_EmiDuf, cr_EmiFull, cr_ErrMes) ){
      _ErrMessage (cr_StdId, cr_ErrMes, &iN_Errs,fh_Err);
      continue; }
...
/* If doing User Specified Equations, do a Check to Validate each eq numbers */
/*  the inputed eq numbers will have been put into the s_CI                  */ 
    strcpy ( s_CI.cr_BatchEqu, cr_BatchEqu);
    if ( !stricmp( s_CI.cr_BatchEqu, "Yes" ) ) {
      if ( !BUE_Duff (&s_CI, &f, &f, &f, cr_ErrMes) ) { 
       _ErrMessage (cr_StdId, cr_ErrMes, &iN_Errs,fh_Err);
        continue; }

      if ( !BUE_Herb (&s_CI, &f, &i_Equ, cr_ErrMes)) {
       _ErrMessage (cr_StdId, cr_ErrMes, &iN_Errs,fh_Err);
        continue; }

      if ( s_CI.i_EqShrub > 0 ) {   /* check if we have a non-missing eq # */
        if ( Shrub_Equ(&s_CI, s_CI.i_EqShrub) < 0 ) {  
           sprintf (cr_ErrMes, "Invalid Shrub Equation: %d\n", s_CI.i_EqShrub);
           strcat  (cr_ErrMes, "    Valid Numbers: 23, 231, 232, 233, 234, 235, 236"); 
          _ErrMessage (cr_StdId, cr_ErrMes, &iN_Errs,fh_Err);
           continue; } }  

      if ( BUE_Litter (&s_CI, &f, &i_Equ) == -1 ) {
         sprintf (cr_ErrMes, "Invalid Litter Equation: %d\n", s_CI.i_EqLit); 
         strcat  (cr_ErrMes, "    Valid Numbers: 997, 998, 999"); 
        _ErrMessage (cr_StdId, cr_ErrMes, &iN_Errs,fh_Err);
         continue; }  
    }  /* if BatchEq - Yes */
...
/* Soil Heating, if the switch is on do it                                    */
     if ( iS_Soil == 1 ) {                   /* Soil Switch is On            */
       ...
       s_SI.f_DufLoaPre = s_CO.f_DufPre;
       s_SI.f_DufMoi = s_CI.f_MoistDuff;

/* If the Batch is running with User Equation numbers than DL said to use */
/* the Duff Depth Reduction percent - SEE Note-7 above */
       if ( !stricmp (s_CI.cr_BatchEqu, "No" ) )
         s_SI.f_DufConPer = s_CO.f_DufPer;
       else
         s_SI.f_DufConPer = s_CO.f_DufDepPer; 


/* Heat Efficencies, when doing GUI FOFEM these would come from the Settings Wnd */
/* they're something JR was experimenting with, so for now for the batch I'm just */
/* going to use the default settings, might have to include them in the batch */
/* input file if JR decides he wants to do that at some point */ 
      s_SI.f_SoilDuffEff = atof (e_EffSoilDuff);  
      s_SI.f_SoilWlEff = atof (e_EffSoilWL); 
      s_SI.f_SoilHsEff = atof (e_EffSoilHS);
...
Put:
     BOF_Put (cr_StdId, &s_CO, &s_CI, iS_Soil, &s_SO);
    *iN = *iN + 1;
...
/* Duff ..................... */
  ...
  strcpy (a_CI->cr_EmiFull, cr_EmiFull); 
...
void _ErrMessage (char cr_StdId[], char cr_ErrMes[], int *aiN_Errs, FILE *fh_Err)
{
 ...
 if ( fh_Err == NULL ) {
   ...
   printf ("    (make sure input file format is correct for selected Model)\n"); 
   printf ("    No Output will be created for this stand.\n"); }
 else {
   fprintf (fh_Err, "\n ERROR - Invalid Input, Stand Id: %s \n    %s \n", cr_StdId,cr_ErrMes);
   fprintf (fh_Err, "    (make sure input file format is correct for selected Model)\n"); 
...
* Note-1: The only error we'd get back is for species not having an Eq
...
{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}{*}*/
int  BAT_MortExec_Old (char cr_InFN[], char cr_OutFN[], int *aiN_Std, int iS_CL, FILE *fh_Err, int *ai_PFI)
{
int i, iN_Err, iN_Std, iN_Rec, iS_PFI; 
float f, f_Prop; 
...
/* Put out the stand totals . . . . . . . . .*/
     if ( strcmp(cr_CurStd,s_SIF.cr_StdId)){ /* Stand Id Changed             */
        BOF_PutMort(cr_CurStd);       /* do output stand record       */
...
/* Prefire . . . . . . . . . . . . . . . . . . .*/
     if ( !SIF_isPFI (&s_SIF)) {
       ...
       if ( !_MortErrMes (&s_SIF,cr_ErrMes,&iN_Err,fh_Err)) /* Chk error messe & disp */
         goto NextRec;
       MRT_Calc (&s_MIS,&s_MI, &s_MO,cr_ErrMes);       /* Calc & accum, Mort,Basal,etc */
      _MortErrMes (&s_SIF,cr_ErrMes,&iN_Err,fh_Err); /* Chk error messe & disp */
       ISS_Init(&s_ISS); 
       strcpy (s_ISS.cr_Spe,s_MI.cr_Spe);
       s_ISS.f_DBH = s_MI.f_DBH;
       s_ISS.i_TreHgt = (int) s_MI.f_Hgt;
       s_ISS.i_Den = (int) s_MI.f_Den;
       s_ISS.i_Killed = (int) s_MO.f_Killed;
       ISS_Set (&s_ISS);
       goto NextRec; }
...
/* Post-Fire Injury . . . . . . . . . . . . . .*/
    if ( SIF_isPFI (&s_SIF)) {
      Chk_SIF_PFI (&s_SIF, &s_PFI, cr_ErrMes);
      if ( !_MortErrMes (&s_SIF,cr_ErrMes,&iN_Err,fh_Err)) /* Chk error messe & disp */
         goto NextRec;   
      if ( !SMT_PFIEqu (s_PFI.cr_Spe, s_PFI.cr_Eq)) {
         _MortErrMes (&s_SIF,"Species has no PFI equation",&iN_Err,fh_Err); 
         goto NextRec; }
      PFI_Calc_Old2 (&s_PFI, cr_ErrMes);  /* See Note-1 above - concerning and error */
      ISS_Init(&s_ISS);
      strcpy (s_ISS.cr_Spe,s_PFI.cr_Spe);
      s_ISS.f_DBH = s_PFI.f_DBH; 
      s_ISS.i_TreHgt = 0;            /* Stick something in, PFI doesn't input a tree hgt */
      s_ISS.i_Den = (int)s_PFI.f_Den;

      if ( s_PFI.f_PMCut < 0 ) {
         f = s_PFI.f_Den * s_PFI.f_Prop    ;
         f += 0.5;    
         s_ISS.i_Killed = (int) f; }
      else if ( s_PFI.f_Prop >= s_PFI.f_PMCut )
        s_ISS.i_Killed = (int) s_PFI.f_Den;
      else 
        s_ISS.i_Killed = 0; 
...
/* Incase we don't catch this before hand */
   _MortErrMes (&s_SIF,"Invalid input record format, can't tell if record is Pre or Post Fire type.",&iN_Err,fh_Err); 
...
Close:
  ...
  *ai_PFI = iS_PFI;                          /* Number of PFI records if any processed */
...
/* if its a Post-fire Injury */
  if ( SIF_isPFI (a_SIF) ) {
    if ( !_ChkCM (a_SIF->f_pDen,    "PFI Density",            cr_ErrMes)) return 0;
    if ( !_ChkCM (a_SIF->f_pCrnDam, "PFI Crown Damage",       cr_ErrMes)) return 0;
    if ( !_ChkCM (a_SIF->f_pDBH,    "PFI DBH",                cr_ErrMes)) return 0;
    if ( !_ChkCM (a_SIF->f_pCKR,    "PFI Cambium Kill Ratio", cr_ErrMes)) return 0;
    if ( !_ChkPMCut (a_SIF->f_pPMCut, cr_ErrMes) ) return 0; 
...
/*********************************************************
* Name: _Chk_PMCut
* Desc: check the Post Fire Injury cutoff point value
*       0 -> 1.0 is a valid post mortality cutoff 
*       if user isn't using a cutoff they can leave the
*       column blank or put a negative number in, so
*       that means anything <= 1.0 is valid
*********************************************************/
int _ChkPMCut (float f_PMCut, char cr_ErrMes[]) 
{
   strcpy (cr_ErrMes,"");
   ...
   sprintf (cr_ErrMes, "Invalid PFI Probability Mortality Cutoff: %f",f_PMCut);    
   return 0; 
}
...
/*******************************************************************
* Name: Chk_SIF_PFI
* Desc: Postfire Mortality Injury
...
int Chk_SIF_PFI (d_SIF *a_SIF, d_PFI *a_PFI, char cr[])
{
  ...
  if ( a_SIF->f_pDen < 1 || a_SIF->f_pDen > 10000 ) {
    sprintf (cr,"Density:%5.0f is out of Range ( 1 -> 10000 )",a_SIF->f_pDen);
    return 0;  }

  if ( a_SIF->f_pCrnDam < 0 || a_SIF->f_pCrnDam > 100 ) {
    sprintf (cr,"Crown Damage Percent: %5.0f is out of Range ( 1 -> 100 )",a_SIF->f_pCrnDam);
    return 0;  }

  if ( a_SIF->f_pDBH < 1 || a_SIF->f_pDBH > 999 ) {
    sprintf (cr,"DBH:%5.0f is out of Range ( 1 -> 999 )",a_SIF->f_pDBH);
    return 0;  }

  if ( a_SIF->f_pCKR < 0 || a_SIF->f_pCKR > 4.0 ) {
    sprintf (cr,"Cambium Kill Ratio :%5.0f is out of Range ( 0 -> 4 )",a_SIF->f_pCKR);
    return 0;  }

  if ( a_SIF->f_pPMCut > 1.0 ) {
    sprintf (cr,"Propability Mortality Cuttoff:%5.0f is out of Range\n Use: 0 -> 1.0, or if not used leave blank or enter a negative value",a_SIF->f_pPMCut);
    return 0;  }

  if ( !stricmp (a_SIF->cr_pBtlKil, "Y" )) goto OK; 
  if ( !stricmp (a_SIF->cr_pBtlKil, "N" )) goto OK; 
  if ( !stricmp (a_SIF->cr_pBtlKil, "X" )) goto OK; 
  
  sprintf (cr,"Beetle Kill: %s,  use ->  Y y  N n  X x",a_SIF->cr_pBtlKil);
  return 0;

OK:
  strcpy (a_PFI->cr_Spe, a_SIF->cr_Spe); 
  a_PFI->f_Den = a_SIF->f_pDen; 
  a_PFI->f_CRP = a_SIF->f_pCrnDam;
  a_PFI->f_DBH = a_SIF->f_pDBH; 
  a_PFI->f_CKR = a_SIF->f_pCKR; 
  strcpy (a_PFI->cr_Btl, a_SIF->cr_pBtlKil);
  a_PFI->f_PMCut = a_SIF->f_pPMCut; 
...
int _ChkLim1000 (d_CI *a_CI, char cr_ErrMes[], char cr_1kFmt[])
{
  ...
  if ( a_CI->f_pcRot < 0 || a_CI->f_pcRot > 100 ) { 
     sprintf (cr_ErrMes,"Down Wood 1000 Hour Load - percent rotten (%4.1f) is out of limits (0 -> 100)", a_CI->f_pcRot);
     return 0; }
...
*       to get used.
* NOTE: A lot of other input codes get checked later in code, even the
...
int Bat_CovGrp (d_CI *a_CI, char cr_ErrMes[])
{
  ...
  if ( !stricmp (a_CI->cr_CoverGroup,e_ShrubGroupChaparral )) return 1; 
  ...
  if ( !stricmp (a_CI->cr_CoverGroup,e_PinFlaWoo )) return 1;

  if ( !stricmp (a_CI->cr_CoverGroup, e_CVT_ShrubGroup )) {
       strcpy (a_CI->cr_CoverGroup, e_ShrubGroup);
       return 1; }

  if ( !stricmp (a_CI->cr_CoverGroup, e_CVT_ShrubGroupChaparral )) {
       strcpy (a_CI->cr_CoverGroup, e_ShrubGroupChaparral);
       return 1; }
  ...
  if ( !stricmp (a_CI->cr_CoverGroup, e_CVT_PinFlaWoo )) {
     strcpy (a_CI->cr_CoverGroup, e_PinFlaWoo);
     return 1; }
...
/* Now append */
 ...
 _StrToChar (Str,cr_Out); 
        ...
        Saving this old function that created a consumed sample batch file 
...
//    fprintf (fh,"# \n");
//    fprintf (fh,"# Duff Method - (moisture method)\n");
//    fprintf (fh,"#  \"%s\", \"%s\", \"%s\", \"%s\" \n", e_Entire, e_Lower, e_NFDR,e_Adj_NFDR);
//    fprintf (fh,"# \n");
...
//    fprintf (fh,"#  \"%s\", \"%s\", \"%s\", \"%s\" \n", e_VeryDry, e_Dry, e_Moderate,e_Wet);
 //   fprintf (fh,"# \n");
  ...
  if ( iS_Soil == 1 ) {
    ...
    fprintf (fh,"#  Id      Lit  Hr  Hr Moist  Hr  Load Moist  %cRot Distr.    Load Moist Dep  Herb Shrub  Foli Brnch %cBrn     Region       Group        Season     Category   Type              Moist\n",'%','%');
    fprintf (fh,"\"stand-1\", .5,  1,   1,  16,   1,   1,   30,   50, \"Even\",     1,    75,  1,    1,   1,    1,    1,   50, \"PacificWest\", \"GrassGroup\", \"Spring\",  \"Natural\", \"Loamy-Skeletal\",   10\n");
    fprintf (fh,"\"stand-2\",  1,  1,   1,  16,   1,   1,   30,   50, \"Right\",    1,    75,  1,    1,   1,    1,    1,   50, \"PacificWest\", \"REDJACPIN\",  \"Spring\",  \"Natural\", \"Fine-Silt\",        15\n");
    fprintf (fh,"\"stand-3\",  1,  1.3, 1,  16,   1,   1,   30,   50, \"Left\",     1,    75,  1,    1,   1,    1,    1,   50, \"PacificWest\", \"Ponderosa\",  \"Fall\",    \"Natural\", \"Fine\",             20\n");
    fprintf (fh,"\"stand-4\",  1,  1,   1,  16,   1,   1,   30,   50, \"End\",      1,    75,  1,    1,   1,    1,    1,   50, \"PacificWest\", \"REDJACPIN\",  \"Spring\",  \"Natural\", \"Coarse-Silt\",      25\n");
    fprintf (fh,"\"stand-5\",  1,  1,   1,  16,   1,   1,   30,   50, \"Center\",   1,    75,  1,    1,   1,    1,    1,   50, \"PacificWest\", \"\",           \"Spring\",  \"Natural\", \"Coarse-Loamy\",     10\n");   }
  else {
