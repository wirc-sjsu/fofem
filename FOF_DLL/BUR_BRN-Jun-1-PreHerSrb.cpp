...
double  _Consumed1000k (int iS_Init, float *af_TPA, double d_Time, d_ES *a_ES);
...
float f_ConHSFB_TPA;
   ...
   f_ConHSFB_TPA = f_ConHerb + f_ConShrb + f_ConBraFol;
...
/*.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.- */
   ...
   _Consumed1000k (1,&f,0,&s_ES);                   /* Init for collecting time step info */ 
...
/* Do the First Time step - 60 Seconds                                       */
   ...
   BOV_Set_fiSHFB (d_1FI - fi);                 /* sav fir int of just Sh,he,fo.br */
...
//     ES_Calc_NEW (&s_ES, d_ConWooLit,d_DufCon,   /* Calc the emissions           */
//              0.0, d_Time,fi);
...
FILE *outfile;
   ...
   if ( strcmp ( cr_LoadFN,"" )) {
     ...
     fprintf(outfile, "       4: Time utill ignition, (%s)\n", e_Second);
     fprintf(outfile, "       5: Time utill burnout, (%s) \n", e_Second);

     fprintf(outfile, "       6: Moisture content, fraction\n");
     fprintf(outfile, "       7: Sigma, surface area to volume ratio, (m.1)  \n");
...
char cr[200];
   ...
   for (i = 0; i < number; i++) {
     ...
     if ( tpig[i]<=tig1 || tpig[i]>=tig2) {
       sprintf (cr_ErrMes, "Burnup: ignition temperature out of range (%s): %8.2f, Low:%8.2f Up:%8.2f", e_Celsis, tpig[i],tig1,tig2);
       return false; }

     if ( tchar[i] <=tch1 || tchar[i]>=tch2) {
       sprintf (cr_ErrMes, "Burnup: char end pyrolisis temperature out of range (%s): %8.2f, Low:%8.2f Up:%8.2f", e_Celsis, tchar[i],tch1,tch2);
       return false; }
   }
   ...
   if ( fi < fir1 || fi > fir2)
      ...
      sprintf (cr_ErrMes, "Burnup: ambient temperature out of range (%s)", e_Celsis);
...
double  d_noxf,  d_noxs, d_so2f, d_so2s;
...
/* Change 9-3-03 ..............................................              */
/* New emission factors for NOX and SO2                                      */
   d_noxf =  3.2;                  /* NOX & SO2 emission factors             */
   d_noxs =  0;
   d_so2f =  1.0;
   d_so2s =  1.0;
...
/* Emissions for current single time step, combine Flame and Smolder....... */
   ...
   a_ES->d_SO2    = (d_so2f  * d_FlaCon) + (d_so2s  * d_SmoCon) ;
...
/*...........................................................*/
/* Individual Flame & Smolder at each time step */
...
*         shouldn't happen because the amounts a calculated from the same
...
/*----------------------------------------------------------*/
/* Flaming Fuels */
   d_Coarse = _Consumed1000k (0, &f_ConTPA,d_time, a_ES); /* Amt 1k coarse wood consumed */
...
d_SGV s_SGV;
   ...
   s_SGV.f_Flame = (float) a_ES->d_Flame;
   s_SGV.f_Coarse = (float) a_ES->d_Coarse;
   s_SGV.f_Duff   = (float) a_ES->d_Duff; 
...
{
   if (strstr(cr, e_swMAX_TIMES    ))
     return 1;
   if (strstr(cr, e_swINTENSITY    )) 
     return 1;
   if (strstr(cr, e_swIG_TIME      ))
     return 1;
   if (strstr(cr, e_swWINDSPEED    )) 
     return 1;
   if (strstr(cr, e_swDEPTH        )) 
     return 1;
   if (strstr(cr, e_swAMBIENT_TEMP )) 
     return 1;
   if (strstr(cr, e_swR0           )) 
     return 1;
   if (strstr(cr, e_swDR           )) 
     return 1;
   if (strstr(cr, e_swTIMESTEP     ))
     return 1;
   if (strstr(cr, e_swDUFF_LOAD    ))
     return 1;
   if (strstr(cr, e_swDUFF_MOIST   ))
     return 1;
...
/**************************************************************
* Name: Consumed1000k
* Desc: Return the amount of 1000k ( 3+ inch) this is consumed
*        for each time step.
...
*         array which is the SAV Surface to Air Volume ratio, 
...
int _SetFuel (int i, double d_Rem, double d_Con, d_ES *a_ES );
...
double s, ti, ts, to, tf, wd;
   ...
   if ( iS_Init == 1 ) {
     gf_TotConTPA = 0 ;
     for ( i = 0; i < MAXNO; i++ ) 
       dr_Lst[i] = dr_Cur[i] = dr_Dif[i] = dr_Done[i] = 0;  
   }
   rem = 0; 
...
/* This loop code was taken from the Summary() function in this file */
   for ( m = 1; m <= number; m++) {     /* for each fuel componet */
     ...
     for ( n = 0; n <= m; n++) {       /* get each entry for a component */
       ...
       wd = wo[mn];                    /* Get the remaining fuel load */
       rem = rem + wd; 
	 }
   ...
   for ( i = 0; i < number; i++ )
     _SetFuel (i, dr_Lst[i], dr_Dif[i], a_ES );
   ...
   d_Coarse = 0;
   d_SAV = e_SURat_DWk_3_6 + 0.001;     /* See Note-2 above */
   for ( i = 0; i < number; i++ ) { 
     s = sigma[i];
     if ( s <= d_SAV  )
       d_Coarse += dr_Dif[i];
