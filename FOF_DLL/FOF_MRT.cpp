...
d_MI s_MI; 
  ...
  if ( !stricmp (a_MIS->cr_EquTyp, e_CroSco) ) {      /* Crown Scorch Equations */
    f = MRT_Calc(a_MIS, &s_MI,a_MO,cr_ErrMes);
    if ( f < 0 ) 
      return -1; 
    return f;  }

  if ( !stricmp (a_MIS->cr_EquTyp, e_CroDam) ) {     /* Crown Damage Equations */
    f = PFI_Calc(a_MIS, a_MO, cr_ErrMes); 
    return f; }
 
  if ( !stricmp (a_MIS->cr_EquTyp, e_BolCha) ) {     /* Bole Char Equations */
...
char cr_Equ[100];
   ...
   for ( i = 0; i < 1000; i++ ) {
     if (  !stricmp (sr_EFR[i].cr_MrtEqu,"") ) 
        break;
     if ( !stricmp (sr_EFR[i].cr_MrtEqu, cr_Equ ) ){  
...
char cr[100]; 
   ...
   strcpy (cr_MrtEqu,""); 
   for ( i = 0; i < 1000; i++ ) {
     if ( !stricmp (sr_SMT[i].cr_Spe,"" ) ) /* End of table */
       return 0;                            
     if ( stricmp (sr_SMT[i].cr_Spe, cr_Spe) ) 
       continue; 
/* if Species matches, now check equation type */
       _EquTyp (sr_SMT[i].cr_MrtEqu, cr);
        if ( !stricmp (cr_EquTyp, cr) ) {    
...
/*.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.-.*/
   for ( i = 0; i < 1000; i++ ){        /* look for entry we want.           */
      ...
      if ( stricmp (sr_EFR[i].cr_MrtEqu,a_MIS->cr_Eq))    /* Not found              */
...
/* DBH and Beetle Kill might be required, so look in Required Field String */
   ...
   if  ( strstr (cr_ReqFld, "btl" ) ) {
     if ( stricmp (a_MIS->cr_BeeDam, "Y") && stricmp (a_MIS->cr_BeeDam, "N") ) {
...
float f,cs,CKR,DBH,btl;
  ...
  if ( !stricmp (a_MIS->cr_BeeDam,e_BtlYes) )
    btl =  1.0;
  else
    btl =  -1.0;
...
float f,cs,DBH,CKR,Btl,F;
  ...
  if ( !stricmp (a_MIS->cr_BeeDam, e_BtlYes)) //  a_PFI->cr_Btl ,e_BtlYes) )
     Btl = 1.0;
  else
     Btl =  0.0;
