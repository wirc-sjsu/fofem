...
char cr[200];
   ...
   for (i = 0; i < number; i++) {
     ...
     if ( tpig[i]<=tig1 || tpig[i]>=tig2) {
       sprintf (cr_ErrMes, "Burnup: ignition temperature out of range (%s): %8.2f, Low:%8.2f Up:%8.2f", e_Celsius, tpig[i],tig1,tig2);
       return false; }

     if ( tchar[i] <=tch1 || tchar[i]>=tch2) {
       sprintf (cr_ErrMes, "Burnup: char end pyrolisis temperature out of range (%s): %8.2f, Low:%8.2f Up:%8.2f", e_Celsius, tchar[i],tch1,tch2);
       return false; }
   }
...
// change 6-30-15
// _Consumed1000k() gets the consumed amts of wood at each time step and 
//  sets them in the ES, later gets put on Smoke Report - Burnout times
// This wasn't originally in this function, it was implement with the new
//  extensive emission stuff, it was added into here when we reverted 
//  back to this old way of doing emissions. 
double d_Coarse;
float  f_ConTPA;
...
// test...............
/* Amounts consumed by fire, smolder and duff */
//   a_ES->d_FlameCon  = ( d_WooLit * d_pcFla ) + d_HSFB;
//   a_ES->d_SmolderCon = (d_WooLit * d_pcSmo);   /* Smolder */
// test...................
...
float f, s; 
   ...
   *af_1kSmo = a->d_cn_1kHr * a->d_cn_1HrSmoPerCent;
   *af_1kFla = a->d_cn_1kHr * (1.0 - a->d_cn_1HrSmoPerCent); 
...
double  dr_Dif[MAXNO];
double  gf_TotConTPA;
// double  dr_Done[MAXNO];  move to top of file 
